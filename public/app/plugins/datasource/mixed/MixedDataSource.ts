...
import { catchError, map, mergeAll, mergeMap, reduce, scan, toArray } from 'rxjs/operators';
...
export class MixedDatasource extends DataSourceApi<DataQuery> {
  ...
  query(request: DataQueryRequest<DataQuery>): Observable<DataQueryResponse> {
    const queries = request.targets.filter((target: DataQuery) => {
      return target.datasource?.uid !== MIXED_DATASOURCE_NAME;
    });
  ...
  private runStreamingBatch(
    ...
    return new Observable<MixedStreamingEvent>((observer) => {
      const dsRequest = cloneDeep(request);
  ...
  private toStreamingResponse(state: MixedStreamingState, requestId?: string): DataQueryResponse {
    ...
    const hasAnyResponse = state.batches.some((batch) => batch.response !== undefined);
...
function reduceStreamingState(batchCount: number, requestId?: string) {
  return scan((state: MixedStreamingState, event: MixedStreamingEvent) => {
