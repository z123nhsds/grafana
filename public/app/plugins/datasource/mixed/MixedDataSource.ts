...
export class MixedDatasource extends DataSourceApi<DataQuery> {
  ...
  query(request: DataQueryRequest<DataQuery>): Observable<DataQueryResponse> {
    ...
    for (const key in sets) {
      if (key === SHARED_DASHBOARD_QUERY) {
        sets[key].forEach((query: DataQuery) => {
  ...
  private toStreamingResponse(state: MixedStreamingState, requestId?: string): DataQueryResponse {
    ...
    const data = state.batches.flatMap((batch: MixedStreamingBatchState) => {
    ...
    const hasStreamingResponse = state.batches.some((batch: MixedStreamingBatchState) => batch.response?.state === LoadingState.Streaming);
    const allCompleted = state.batches.every((batch: MixedStreamingBatchState) => batch.completed);
